  ...
  <xsl:for-each select="
    ...
    <xsl:choose>
      <xsl:when test="@kind='class' or @kind='struct'">
        <xsl:if test="
            contains(compoundname, 'backbone::') and
...
<xsl:template name="strip-backbone-ns">
  <xsl:param name="name"/>
  <xsl:choose>
    <xsl:when test="contains($name, 'backbone::')">
      <xsl:value-of select="substring-after($name, 'backbone::')"/>
    </xsl:when>
    <xsl:otherwise>
...
<xsl:template match="ref[@kindref='compound']" mode="markup">
  ...
  <xsl:choose>
    <xsl:when test="contains(@refid, 'backbone') or contains($name, 'backbone::')">
      <xsl:variable name="dox-ref-id" select="@refid"/>
      <xsl:variable name="ref-name">
        <xsl:call-template name="strip-backbone-ns">
          <xsl:with-param name="name"
      ...
      <xsl:text>[link libbackbone_cpp.reference.</xsl:text>
...
<xsl:template name="header-requirements">
  ...
  <xsl:text>[^backbone++/</xsl:text>
  <xsl:value-of select="substring-after($file, 'backbone++/')"/>
  <xsl:text>]</xsl:text>
  <xsl:value-of select="$newline"/>
</xsl:template>
...
<xsl:text>    </xsl:text><xsl:if test="@prot='public'">public </xsl:if><xsl:call-template
 name="strip-backbone-ns"><xsl:with-param name="name" select="."/></xsl:call-template><xsl:if
...
<xsl:choose>
  <xsl:when test="count(name) &gt; 0">
    [[link libbackbone_cpp.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="name"/>
  ...
  <xsl:otherwise>
    ...
    [[link libbackbone_cpp.reference.<xsl:value-of select="$type-id"/>
...
<xsl:for-each select="sectiondef[@kind='public-func' or @kind='public-static-func']/memberdef" mode="class-table">
  ...
  [
    [[link libbackbone_cpp.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of select="$id"/>
...
</xsl:if>``[link libbackbone_cpp.reference.<xsl:value-of select="$class-id"/>.<xsl:value-of
...
['Inherited from <xsl:call-template name="strip-backbone-ns">
...
<xsl:text>
      ...
      <xsl:choose>
        ...
        <xsl:when test="declname = 'SocketService1' or declname = 'SocketService2'">
          <xsl:value-of select="concat('``[link libbackbone_cpp.reference.SocketService ', declname, ']``')"/>
        </xsl:when>
        ...
        <xsl:otherwise>
          <xsl:value-of select="concat('``[link libbackbone_cpp.reference.', declname, ' ', declname, ']``')"/>
        </xsl:otherwise>
      </xsl:choose>
...
<xsl:text>  </xsl:text><xsl:if test="string-length($stripped-type) &gt; 0"><xsl:value-of
 select="$stripped-type"/><xsl:text> </xsl:text></xsl:if>``[link libbackbone_cpp.reference.<xsl:value-of
